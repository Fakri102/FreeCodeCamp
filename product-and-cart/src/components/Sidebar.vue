<template>
     <aside class="cart-container">
      <div class="cart">
        <h1 class="cart-title spread">
          <span>
            Cart
            <i class="icofont-cart-alt icofont-1x"></i>
          </span>
          <button @click="toogle" class="cart-close">&times;</button>
        </h1>

        <div class="cart-body">
          <table class="cart-table">
            <thead>
              <tr>
                <th><span class="sr-only">Product Image</span></th>
                <th>Product</th>
                <th>Price</th>
                <th>Qty</th>
                <th>Total</th>
                <th><span class="sr-only">Actions</span></th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(product, key, idx) in cart" :key="idx">
                <td><i class="icofont-carrot icofont-3x"></i></td>
                <td>{{ key }}</td>
                <td>${{ getProductPrice(key)  }}</td>
                <td class="center">{{ product }}</td>
                <td>${{  getProductTotalPrice(key) }}</td>
                <td class="center">
                  <button @click="remove(key)" class="btn btn-light cart-remove">
                    &times;
                  </button>
                </td>
              </tr>
            </tbody>
          </table>

          <p class="center" v-if="!Object.keys(cart).length"><em>No items in cart</em></p>
          <div class="spread">
            <span><strong>Total:</strong>${{ getTotalCartPrice() }}</span>
            <button class="btn btn-light">Checkout</button>
          </div>
        </div>
      </div>
    </aside>
</template>

<script>
export default {
      props: ['toogle', 'cart', 'inventory', 'remove'],
    computed: {
      cartTotal() {
        return (this.cart.carrots * 4.82).toFixed(2)
      }
    },
    methods: {
      getProductPrice(name) {
        const product = this.inventory.find((p) => {
          return p.name === name
        })
        return product.price.USD;
      },

      getProductTotalPrice(name) {
        const productTotalPrice = this.getProductPrice(name)
        return (productTotalPrice * this.cart[name]).toFixed(2);
      },

      getTotalCartPrice() {
        //[key, value]
        const totalPrice = Object.entries(this.cart).reduce((acc, current, index) => {
          return acc + (current[1] * this.getProductPrice(current[0]))
        }, 0)
        return totalPrice.toFixed(2);
      },
    },
}
</script>